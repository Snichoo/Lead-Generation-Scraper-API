...
async function getContactsFromApify(domains) {
...
function extractSuburbOrCity(locationInput) {
...
async function scrapeGoogleMaps(businessType, location, leadCount) {
...
async function runActorPool(businessType, suburbs, maxConcurrency, stateAbbr, leadCount) {
    const allResults = [];
...
async function generateCSVFile(businessType, location, data) {
    if (data.length === 0) {
...
export async function generateLeads(businessType, locationInput, leadCount) {
    try {
        ...
        if (locationCheckResult === "yes") {
            ...
            if (suburbsByCity[cityKey]) {
                ...
                uniqueResults = await runActorPool(businessType, structuredSuburbs, 50, stateAbbr, leadCount);
        ...
        let savedData = readJsonFromFile("finalResults.json");
        ...
        for (let index = 0; index < savedData.length; index++) {
            ...
            if (company.website) {
                const websiteDomain = getRootDomain(new URL(company.website).hostname.toLowerCase());
            ...
            for (const domain of organizationDomains) {
                ...
                const completion = await openai.beta.chat.completions.parse({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are a helpful assistant that identifies the person with the highest role in a company based on their job title.',
                        },
                        {
                            role: 'user',
                            content: `Given the following people and their job titles: ${JSON.stringify(cleanedResults)}. Find the person with the highest role and provide their name and job title.`,
                        },
                    ],
                ...
                if (highestRolePerson) {
                    ...
                    const personDetails = contacts.find((c) => `${c.first_name || ''} ${c.last_name || ''}`.trim() === highestRolePerson.name &&
        ...
        for (let index = 0; index < savedData.length; index++) {
            const company = savedData[index];
            if ((!company.company_personal_email ||
                    ...
                    (async () => {
                        try {
                            ...
                            if (emails && emails.length > 0) {
                                ...
                                if (!companyInSavedData.company_general_email ||
                                    companyInSavedData.company_general_email.trim() === '') {
                                    companyInSavedData.company_general_email = emails[0]; // Take the first email
                                    console.log(`Added general email to company at index ${idx}: ${companyInSavedData.company_general_email}`);
                                }
                            }
                            else {
...
async function checkLocation(location) {
    ...
    try {
        ...
        normalizedLocation = normalizedLocation.replace(/\b(australia|nsw|vic|qld|sa|wa|tas|nt|act)\b/gi, '');
